...
namespace WindowsFormsApp1
{
    public partial class Form1 : Form
    {
        ...
        {
            { nameof(Employee.EmplName), "직원 이름" },
            { nameof(Employee.GetDisplayUidnum7), "주민등록번호 (앞 7자리)" },
            { nameof(Employee.EmplNum), "직원 번호" },
            { nameof(Employee.SalaryBname), "은행 이름" },
            { nameof(Employee.SalaryAcctnum), "계좌 번호" },
            { nameof(Employee.SalaryAmt), "급여 금액" },
            { nameof(Employee.SalaryBaseYear), "기준 연도" },
            { nameof(Employee.EmploymentDate), "고용 날짜" },
            { nameof(Employee.GetDisplayIsSafe), "안전 여부 (O/X)" }
        };
